...
router.post('/', (req, res) => {
  ...
  const { event_name, event_dates, event_description, ward, stake, leader_name, leader_phone, leader_email, notify_email, notify_phone, notify_carrier } = req.body;
  ...
  db.prepare(`INSERT INTO events (id, created_by, event_name, event_dates, event_description, ward, stake, leader_name, leader_phone, leader_email, notify_email, notify_phone, notify_carrier) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(id, req.user.id, sanitizeString(event_name), sanitizeString(event_dates), sanitizeString(event_description, 1000), sanitizeString(ward), sanitizeString(stake), sanitizeString(leader_name), sanitizeString(leader_phone), sanitizeString(leader_email), notify_email || null, notify_phone || null, notify_carrier || null);
...
router.put('/:id', (req, res) => {
  ...
  const { event_name, event_dates, event_description, ward, stake, leader_name, leader_phone, leader_email, notify_email, notify_phone, notify_carrier, is_active } = req.body;

  db.prepare(`UPDATE events SET event_name = ?, event_dates = ?, event_description = ?, ward = ?, stake = ?, leader_name = ?, leader_phone = ?, leader_email = ?, notify_email = ?, notify_phone = ?, notify_carrier = ?, is_active = ? WHERE id = ?`)
    .run(
