...
function createEventRepository(): EventRepository {
  return {
    ...
    async listForMe(): Promise<Event[]> {
      const data = await apiFetch('/api/events/for-me');
      return data.events;
    },
    ...
    async getAllSubmissions(filter?: { groupId?: string | null; activityId?: string | null }): Promise<AllSubmission[]> {
      const params = new URLSearchParams();
      if (filter?.groupId) params.set('groupId', filter.groupId);
      if (filter?.activityId) params.set('activityId', filter.activityId);
      const qs = params.toString();
      const data = await apiFetch(`/api/events/all-submissions${qs ? `?${qs}` : ''}`);
      return data.submissions;
    }
  };
}
...
/**
 * The single remote adapter — a thin REST client over the Permish HTTP API
 * (the Bun + Hono server). Backend-agnostic: business logic lives server-side.
 * Replaces the former Express and PocketBase adapters.
 */
export function createHttpRepository(): DataRepository {
  return {
