...
## Known gaps
_None — the 2026-07-14 Fable audit is fully closed (waves 1–4). Deliberately deferred: styling-agent minor inventory (unpersisted specifics; PopoverMenu at 2× is under the 3× rule), per-route status-code normalization, and broad test-coverage expansion for pre-existing untested routes (auth flows beyond login, group mutations, invite branches)._

## Recently Completed
- **Fable audit fixes, wave 4 — cross-cutting server minors + offline gate** (2026-07-14, branch `fix/fable-audit-3`, TODO-019) — global `{error}` JSON handlers (onError/notFound), nosniff/frame/referrer headers, attachment Content-Length pre-check + magic-byte validation, admin password-reset audit row, and local-mode form load/submit now reject deactivated activities (server-410 parity). Server 46/46, frontend 184/184, svelte-check clean. See [[changelog/2026-07-14]].
