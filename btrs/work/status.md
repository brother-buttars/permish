...
## Known gaps
- **Fable audit — remaining Minor backlog** — after waves 1+2, what's left is mostly styling/refactor debt: the remaining atomic-design violations in `event/[id]/submissions` (inline header/filter/list markup), the segmented-tab strip inlined in 3 route layouts + SignaturePad, amber-warning-box vs AlertBox divergence, raw `<select>`s on the invites page, `FormField` molecule unused (label+input+error inlined 22×), SignaturePad canvas resize/re-blit, mobile tap-target sizes in SubmissionListView, Sheet (mobile nav) Escape/trap, and copy/terminology items ("Activity" audit-log tab). See [[evidence/reviews/2026-07-14-fable-full-audit]].
